...
class MySecret:
    ...
    logger.info(f"VIRUS_TOTAL_KEY: {'Obtained' if VT_FILE_UPLOAD_ENDPOINT != 'Endpoint not obtained' else 'Not obtained'}")
    VT_ANALYSIS_ENDPOINT = os.getenv("VT_ANALYSIS_ENDPOINT", 'Endpoint not obtained')
    logger.info(f"VIRUS_TOTAL_KEY: {'Obtained' if VT_ANALYSIS_ENDPOINT != 'Endpoint not obtained' else 'Not obtained'}")
    VT_IP_SCAN_ENDPOINT = os.getenv("VT_IP_SCAN_ENDPOINT", 'Endpoint not obtained')
    logger.info(f"VIRUS_TOTAL_KEY: {'Obtained' if VT_IP_SCAN_ENDPOINT != 'Endpoint not obtained' else 'Not obtained'}")
    URLSCANIO_API_KEY = os.getenv("URLSCANIO_API_KEY", 'Endpoint not obtained')
